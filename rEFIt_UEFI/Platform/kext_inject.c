...
void patch_mkext_v1(LOADER_ENTRY *Entry, UINT8 *drvPtr) {
  ...
  if(!IsListEmpty(&gKextList)) {
    for (Link = gKextList.ForwardLink; Link != &gKextList; Link = Link->ForwardLink) {
      KextEntry = CR(Link, KEXT_ENTRY, Link, KEXT_SIGNATURE);
      MKextFile *mkext_insert = (MKextFile*)((VOID*)mkext_ptr + sizeof(MKextHeader) + mkext_numKexts * sizeof(MKextFile));

      // free some space
      CopyMem((VOID*) mkext_insert + sizeof(MKextFile),
              (VOID*) mkext_insert,
      ...
      for (UINT32 i = 0; i < mkext_numKexts; i++) {
        MKextFile *kext_base = (MKextFile*)((VOID*)mkext_ptr + sizeof(MKextHeader) + i * sizeof(MKextFile));
